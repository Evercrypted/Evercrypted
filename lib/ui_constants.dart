import 'package:flutter/material.dart';

const primaryColor = Color(0xFF00BF6D);
const secondaryColor = Color.fromRGBO(251, 139, 36, 1);
const contentColorLightTheme = Color(0xFF1D1D35);
const contentColorLightThemeSecondary = Color.fromARGB(255, 53, 53, 81);
const lightGrey = Color.fromARGB(255, 162, 162, 162);
const light = Color(0xFFF7F8FC);
const contentColorDarkTheme = Color(0xFFF5FCF9);
const warninngColor = Color(0xFFF3BB1C);
const errorColor = Color.fromARGB(255, 240, 95, 55);

const defaultPadding = 16.0;

const logoTheme = "assets/icons/evercrypted.svg";
